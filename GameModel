...
public class GameModel implements Cloneable {
   ...
   /**
    ...
    /**
     * Create a copy of the constructor/variables
     * @param other
     */
    public GameModel(GameModel other)
    {
    	numberOfSteps = other.numberOfSteps;
    	currentDot = new Point(other.currentDot);
    	sizeOfGame = other.sizeOfGame;
    	generator = new Random();
   ...
   /**
    ...
    /**
     * Clone gameModel, override
     * Return the cloned gameModel
     */
    @Override
    protected GameModel clone() throws CloneNotSupportedException {
    	clone = new GameModel(sizeOfGame);
    	for(int i =0; i<sizeOfGame; i++){
    		for(int j =0; j<sizeOfGame; j++){
    			clone.model[i][j] = model[i][j];
    		}
    		clone.setCurrentDot(currentDot.getX(), currentDot.getY());
    	}
    	return clone;
    }
}
